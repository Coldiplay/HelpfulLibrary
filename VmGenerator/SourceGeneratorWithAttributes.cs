...
public class SourceGeneratorWithAttributes : IIncrementalGenerator
{
    ...
    private readonly ImmutableDictionary<string, AttributeType> attributesTypes = Load();


    private static ImmutableDictionary<string, AttributeType> Load()
    {
        var dict = ImmutableDictionary.Create<string, AttributeType>();

        dict.AddRange(
            [
                new($"{NamespaceAttributes}.{AttributeNameVm}", AttributeType.CreateVm),
                new($"{NamespaceAttributes}.{AttributeNameDto}", AttributeType.CreateDto)
            ]);
...
";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var provider = context.SyntaxProvider
            .CreateSyntaxProvider(
                ...
                (ctx, _) => GetClassDeclaration(ctx, attributesTypes))
            .Where(t => t.Item2 != AttributeType.None);
    ...
    private static (ClassDeclarationSyntax, AttributeType) GetClassDeclaration(
        GeneratorSyntaxContext context, ImmutableDictionary<string, AttributeType> attributeTypes)
    {
        ...
        foreach (AttributeSyntax attributeSyntax in attributeListSyntax.Attributes)
        {
            ...
            if (attributeTypes.TryGetValue(attributeName, out var attributeType))
                return (classDeclarationSyntax, attributeType);
        }

        return (classDeclarationSyntax, AttributeType.None);
    }
    ...
    private void GenerateCode(SourceProductionContext context, Compilation compilation,
        ImmutableArray<(ClassDeclarationSyntax, AttributeType)> classDeclarations)
    {
        // Go through all filtered class declarations.
        foreach (var classDeclarationSyntaxTuple in classDeclarations)
        {
            ClassDeclarationSyntax classDeclarationSyntax = classDeclarationSyntaxTuple.Item1;
            ...
            switch(classDeclarationSyntaxTuple.Item2)
            {
                case AttributeType.CreateVm:
                    GenerateCodeVm(classSymbol, classDeclarationSyntax, classDeclarationSyntaxTuple.Item2, context);
                    break;

                case AttributeType.CreateDto:
                    GenerateCodeDto(classSymbol, classDeclarationSyntax, classDeclarationSyntaxTuple.Item2, context);
                    break;
            }
    ...
    private static readonly Action<INamedTypeSymbol, ClassDeclarationSyntax, AttributeType, SourceProductionContext> GenerateCodeDto = 
        (classSymbol, classDeclarationSyntax, attributeType, context) =>
    {
...
";
    ...
    private static readonly Action<INamedTypeSymbol, ClassDeclarationSyntax, AttributeType, SourceProductionContext> GenerateCodeVm =
        (classSymbol, classDeclarationSyntax, attributeType, context) =>
        {
            var namespaceName = classSymbol.ContainingNamespace.ToDisplayString();
